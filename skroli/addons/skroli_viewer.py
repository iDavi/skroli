...
function saveIngestors(btn){
  btn.disabled = true;
  setStatus('ingestors-status', 'saving…', '');
  postConfig({rss: {feeds: lines('feeds'), subreddits: lines('subreddits')}})
    .then(function(){ setStatus('ingestors-status', 'saved — refresh to fetch new sources', 'ok'); })
    .catch(function(err){ setStatus('ingestors-status', err.message, 'err'); })
    .then(function(){ btn.disabled = false; });
}
function saveEnhancers(btn){
  btn.disabled = true;
  setStatus('enhancers-status', 'saving…', '');
  postConfig({score: {
    half_life_hours: document.querySelector('[name="half_life_hours"]').value,
    weights: document.querySelector('[name="weights"]').value
  }})
    .then(function(){ setStatus('enhancers-status', 'saved — refresh to re-score feed', 'ok'); })
    .catch(function(err){ setStatus('enhancers-status', err.message, 'err'); })
    .then(function(){ btn.disabled = false; });
}
